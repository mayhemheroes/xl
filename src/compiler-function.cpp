...
RECORDER(closures, 128, "Closure/lazy binding flow in O3");
RECORDER(boxed_assign_data, 128, "Boxed tree Data() assignment/read debug");
...
// ----------------------------------------------------------------------------
{
    ...
    case NAME:
    {
        ...
        record(boxed_assign_data,
               "Data(NAME) %t bound %t rewrite %t where %t",
               expr, bound, rewrite, where);
        if (!existing)
        {
            ...
            if (rewrite)
                existing = PatternBase(rewrite->left);
            record(boxed_assign_data,
                   "Data(NAME) recovered pattern %t from rewrite %t",
                   existing, rewrite);
        }
        ...
        if (JIT::Value_p result = Known(existing))
        {
            ...
            result = code.Store(result, ptr);
            record(boxed_assign_data,
                   "Data(NAME) %t storing known %t value %v into %v",
                   expr, existing, result, ptr);
            return result;
        }

        // Arguments not bound here are returned as a constant
        record(boxed_assign_data, "Data(NAME) %t returning ConstantTree(%t)",
               expr, existing);
...
JIT::Value_p CompilerFunction::ThickClosureForExpr(Tree *expr)
// ----------------------------------------------------------------------------
//   Build a JIT-local thick pointer { data*, invoke* } for a delayed tree.
// ----------------------------------------------------------------------------
//   data* points at { Scope*, Tree* }; invoke* is eval_fn for expr, cached
//   per (TypesScope, expr).  Scope SSA is values[scope_type] at this site.
{
    record(closures, "Build thick closure for %t", expr);
    JIT::Value_p scopeVal = values[scope_type];
    if (!scopeVal)
    {
        Ooops("Internal: thick closure needs Scope for $1", expr);
        scopeVal = data.NullConstant(compiler.scopePtrTy);
    }

    CompilerTypes *closureTypes = types;
    if (Name *n = expr->AsName())
    {
        Context *ctx = closureTypes->TypesContext();
        if (!ctx || !ctx->Bound(n, true))
        {
            record(closures,
                   "Thick closure %t switching type context %p -> unit %p",
                   expr, closureTypes, unit.types);
            closureTypes = unit.types;
        }
    }

    JIT::Function_p &slot = unit.CompiledClosure(closureTypes->TypesScope(),
                                                 expr);
    if (!slot)
    {
        record(closures, "Compile closure invoke for %t", expr);
        std::ostringstream os;
        os << "xl.clo." << (void *) expr;
        CompilerEval ce(unit, expr, closureTypes, os.str());
        ce.Compile(expr, true);
        (void) ce.Finalize(false);
        slot = ce.Function();
    }
    else
    {
        record(closures, "Reuse closure invoke for %t as %v", expr, slot);
    }

    JIT::Value_p envPtr = data.Alloca(compiler.closureEnvTy, "clenv");
    JIT::Value_p scopeField =
        code.StructGEP(compiler.closureEnvTy, envPtr, 0, "clenv.sc");
    JIT::Value_p bodyField =
        code.StructGEP(compiler.closureEnvTy, envPtr, 1, "clenv.body");
    code.Store(scopeVal, scopeField);
    code.Store(ConstantTree(expr), bodyField);

    JIT::Value_p thickPtr = data.Alloca(compiler.closureValueTy, "thick");
    JIT::Value_p dataField =
        code.StructGEP(compiler.closureValueTy, thickPtr, 0, "thick.data");
    JIT::Value_p invokeField =
        code.StructGEP(compiler.closureValueTy, thickPtr, 1, "thick.invoke");
    code.Store(envPtr, dataField);
    code.Store(slot, invokeField);
    JIT::Value_p thickVal = code.Load(compiler.closureValueTy,
                                      thickPtr,
                                      "thick.val");
    record(closures, "Built thick closure value %v for %t", thickVal, expr);
    return thickVal;
}


JIT::Value_p CompilerFunction::InvokeThickClosure(Tree *expr,
                                                JIT::Value_p thick,
                                                JIT::Type_p thickTy)
// ----------------------------------------------------------------------------
//   Force-evaluate a thick closure by calling invoke (eval_fn convention).
// ----------------------------------------------------------------------------
//   Returns Tree* (boxed tree pointer) from the callee eval_fn.
{
    record(closures, "Invoke thick closure for %t value %v type %T",
           expr, thick, thickTy);
    JIT::StructType_p sty = compiler.closureValueTy;
    JIT::Value_p agg = thick;
    if (thickTy->isPointerTy())
        agg = code.Load(sty, thick, "thick.ld");

    JIT::Value_p envPtr = code.StructLoad(sty, agg, 0, "cl.envp");
    JIT::Value_p invokeVal = code.StructLoad(sty, agg, 1, "cl.invoke");

    JIT::Value_p scopeGEP =
        code.StructGEP(compiler.closureEnvTy, envPtr, 0, "cl.scpp");
    JIT::Value_p bodyGEP =
        code.StructGEP(compiler.closureEnvTy, envPtr, 1, "cl.bodyp");
    JIT::Value_p scopeV =
        code.Load(compiler.scopePtrTy, scopeGEP, "cl.scope");
    JIT::Value_p bodyV =
        code.Load(compiler.treePtrTy, bodyGEP, "cl.body");

    JIT::Value_p callee = code.IndirectCallable(invokeVal, "cl.invokep");
    JIT::Value_p result = code.Call(compiler.evalTy, callee, scopeV, bodyV);
    record(closures, "Invoked closure %t => %v", expr, result);
    return result;
}
...
// ----------------------------------------------------------------------------
{
    ...
    case NAME:
    {
        ...
        if (!decl)
        {
            Scope_g where = nullptr;
            Rewrite_g rewrite = nullptr;
            (void) ctx->Bound(what, true, &rewrite, &where);
            if (rewrite)
...
// ----------------------------------------------------------------------------
    : CompilerEval(unit, body, types, text("xl.eval"))
{
}
...
CompilerEval::CompilerEval(CompilerUnit &unit,
                           ...
                           const text &name)
// ----------------------------------------------------------------------------
//   Build a compiler eval function with a unique LLVM symbol name
// ----------------------------------------------------------------------------
    : CompilerFunction(unit, body, body, types, unit.compiler.evalTy, name)
{
